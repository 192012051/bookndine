    ...
    init: function (callback) {
        var ready = function () {
            ...
            try {
                ...
                /*
                    Empty Submission Issue #7825: handleFormSubmit isn't being called when we get an empty submission. To
                    debug why this function isn't being called, we need to ensure the event listener is not being removed.
                    If isn't being removed, we also log the event handler calls to check if the event stack is cancelled
                    at any point. 'trackEventHandlers' is set in buildSource and is only enabled for selected enterprise
                    accounts
                */
                if (JotForm.trackEventHandlers) {
                    HTMLFormElement.prototype.addEventListener = (function (originalFn) {
                        return function (type, listener, options) {
                            if (type === 'submit') {
                                var handlerName = (listener.handler && listener.handler.name) || listener.name || 'unnamed-event';

                                // Log each submit event handler to find the last event fired before validation was cancelled
                                listener = (function (originalListener) {
                                    return function () {
                                        trackExecution(handlerName + '-fired');
                            ...
                            return originalFn.apply(this, [type, listener, options]);
                        };
                    })(HTMLFormElement.prototype.addEventListener);

                    HTMLFormElement.prototype.removeEventListener = (function (originalFn) {
                        return function (type, listener, options) {
                            if (type === 'submit') {
                                var handlerName = (listener.handler && listener.handler.name) || listener.name || 'unnamed-event';

                                // Log if remove submit event was called.
                                trackExecution(handlerName + '-removed');
                            }
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                ...
                setTimeout(function (calc) {
                    ...
                    if (!matchForThisResult[condition.action[0].resultField+subfield]) {
                        try {
                            ...
                            if (!JotForm.isEditMode() && (!document.getElementById('draftID') || JotForm.replaceTagTest)) {
    ...
    checkCalculation: function (calc, sourceField, sourceEvent) {
        ...
        if(JotForm.ignoreInsertionCondition && JotForm.replaceTagTest && resultFieldType !== 'html') {
            return;
        }
  ...
  handleSignSignatureInputs: function() {
    ...
    validator: function () {
        ...
        $A(JotForm.forms).each(function (form) { // for each JotForm form on the page
            trackExecution('validator-pre-mount-' + (!!form.validationSet));
            ...
            /*
                Empty Submission Issue #7825: handleFormSubmit isn't being called when we get an empty submission. Add
                this handler to check if non-prototype events are still being called and something else is preventing
                handleFormSubmit from calling.
            */
            var submitEventTest = function () {
                trackExecution('test-submit-event');
            }

            form.addEventListener('submit', submitEventTest);

            var submitButton = document.querySelector('.form-submit-button');

            if (submitButton) {
                submitButton.addEventListener('click', function () {
                    trackExecution('test-click-event');
                });
            }
            ...
            */
            if (form.retrieve) {
                ...
                submitEventMounted = 'proto-missing';
            }

            trackExecution('validator-post-mount-' + submitEventMounted);
    ...
    $A(document.forms).each(function (form, index) {
        if (form.name === "form_" + form.id) {
            ...
            if (submitSourceTrackerElement) {
                var indexString = (index > 0 ? '-' + index : '');
                submitSourceTrackerElement.value = value + indexString + '-' + (new Date()).getTime();
            } else {
                // If the input was somehow removed, create a new input and track original event
                appendHiddenInput('submitSource', value + '-readded-' + (new Date()).getTime());
            }
        }
    });
}
            ...
            if (timeToSubmitElement) {
                var timeToSubmitInterval = setInterval(function () {
                    if (Number(timeToSubmitElement.value) >= 20) {
                        ...
                        timeToSubmitElement.value = Math.round(((new Date()).getTime() - startTime) / 1000);
                    }
                }, 1000);
            }
        }
