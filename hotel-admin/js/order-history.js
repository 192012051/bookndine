...
function fetchBookingHistory() {
	var requestData = {
		hotel_id: JSON.parse(sessionStorage.user_details).hotel_id
	};
...
function changeStatus(obj) {
	...
	var result = confirm("Are you sure you want to Accept this order?");

	if (result) {
		fetch('../php/changeStatus.php', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
...
function loadOrderHistory(bookingData) {
	...
	for (let i = 0; i < bookingData.length; i++) {
		...
		if (order.booking_status == 0) {
			historyContent += '<a class="btn btn-sm btn-outline-primary ms-3" onclick="changeStatus(this)" id="' + order.booking_id + '" href="#"><i class="icofont-headphone-alt"></i>ACCEPT</a>';
		}
